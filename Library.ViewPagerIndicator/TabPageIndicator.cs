...
namespace DK.Ostebaronen.Droid.ViewPagerIndicator
{
    public class TabPageIndicator : HorizontalScrollView, IPageIndicator
    {
        ...
        protected override void OnMeasure(int widthMeasureSpec, int heightMeasureSpec)
        {
            ...
            var lockedExpanded = widthMode == MeasureSpecMode.Unspecified;
            FillViewport = lockedExpanded;

            var childCount = _tabLayout.ChildCount;
            if (childCount > 1 && (widthMode == MeasureSpecMode.Unspecified || widthMode == MeasureSpecMode.AtMost))
            {
                if (childCount > 2)
                    _maxTabWidth = (int)(MeasureSpec.GetSize(widthMeasureSpec) / 2);
